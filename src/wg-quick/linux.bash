...
del_if() {
	...
	if [[ -z $TABLE || $TABLE == auto ]] && get_fwmark table && [[ $(bwg show "$INTERFACE" allowed-ips) =~ /0(\ |$'\n'|$) ]]; then
...
set_mtu_up() {
	...
	done < <(bwg show "$INTERFACE" endpoints)
	if [[ $mtu -eq 0 ]]; then
...
get_fwmark() {
	local fwmark
	fwmark="$(bwg show "$INTERFACE" fwmark)" || return 1
	...
	if type -p iptables >/dev/null; then
		...
		for iptables in iptables ip6tables; do
			...
			while read -r line; do
				[[ $line == "*"* || $line == COMMIT || $line == "-A "*"-m comment --comment \"bwg-quick(8) rule for $INTERFACE\""* ]] || continue
				[[ $line == "-A"* ]] && found=1
...
add_default() {
	...
	if ! get_fwmark table; then
		...
		cmd bwg set "$INTERFACE" fwmark $table
	fi
	...
	local marker="-m comment --comment \"bwg-quick(8) rule for $INTERFACE\"" restore=$'*raw\n' nftable="wg-quick-$INTERFACE" nftcmd
...
set_config() {
	cmd bwg setconf "$INTERFACE" <(echo "$WG_CONFIG")
}
...
save_config() {
	...
	old_umask="$(umask)"
	umask 077
	current_config="$(cmd bwg showconf "$INTERFACE")"
...
cmd_usage() {
	...
	See bwg-quick(8) for more info and examples.
	_EOF
}
...
cmd_down() {
	[[ " $(bwg show interfaces) " == *" $INTERFACE "* ]] || die "\`$INTERFACE' is not a WireGuard interface"
