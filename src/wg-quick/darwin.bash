...
get_real_interface() {
	...
	[[ -f "/var/run/amneziabwg/$INTERFACE.name" ]] || return 1
	interface="$(< "/var/run/amneziabwg/$INTERFACE.name")"
	[[ -n $interface && -S "/var/run/amneziabwg/$interface.sock" ]] || return 1
	diff=$(( $(stat -f %m "/var/run/amneziabwg/$interface.sock" 2>/dev/null || echo 200) - $(stat -f %m "/var/run/wireguard/$INTERFACE.name" 2>/dev/null || echo 100) ))
